...
read -p "What is the number of bug fix in the master branch? Since NEXTGEN diverged from it?" ansmain
...
if [ "$ansmain" == "3" ] ;
then
    echo "$(gettext "Wrong: \"3\" is the total number of commit in the master branch. Not the number since NEXTGEN has branch from it")."
...
then
    echo "$(gettext "This is not correct number for the master branch. Please check and try again.")"
    unset goal current ansmain ansnext
    false
elif [ "$ansnext" == "5" ] ;
then
    echo "Wrong: \"5\" is the total number \
of commit in the NEXTGEN branch. Not the number since NEXTGEN has branch from master"
    unset goal current ansmain ansnext
    false
elif [ "$ansnext" != "3" ] ;
then
